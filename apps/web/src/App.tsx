    ...
    <div className="shell">
      <aside className={mobileNav ? "sidebar open" : "sidebar"}>
        <div className="brand">
          ...
          <button className="close" onClick={() => setMobileNav(false)}>
            <X />
          </button>
        </div>
        <nav>
          ...
          <button
            className={page === "observability" ? "active" : ""}
        ...
        <div className="architecture">
          ...
          <div>
            Planner <ChevronRight /> RAG <ChevronRight /> LLM
          </div>
          <p>
...
function Chat() {
  ...
  async function send(text = input) {
    ...
    const updateAssistant = (update: (message: Conversation) => Conversation) =>
      setMessages((old) =>
        old.map((message) =>
    ...
    try {
      ...
      updateAssistant((message) => ({
        ...
        streaming: false,
        content:
          message.content || (e instanceof Error ? e.message : "服务异常"),
      }));
    } finally {
      setLoading(false);
    }
  ...
  return (
    <section className="chat-page">
      <div className="conversation">
        {!messages.length && (
          <div className="hero">
            ...
            <span>ENTERPRISE RAG AGENT</span>
            <h2>让企业知识，真正参与决策</h2>
            <p>
              检索增强、智能规划、工具调用与长期记忆，在一个可观测的 Agent
              工作流中协同。
            </p>
        ...
        {messages.map((m) => (
          <div key={m.id} className={`message ${m.role}`}>
            <div className="avatar">{m.role === "user" ? "YOU" : <Bot />}</div>
            <div className="bubble">
              {m.events && m.events.length > 0 && (
                <AgentSteps events={m.events} />
              )}
              <div className={m.streaming ? "answer streaming" : "answer"}>
                {m.content || "正在连接 Agent…"}
              </div>
              {m.events?.map((event) =>
                event.type === "answer" && event.citations.length > 0 ? (
                  <div className="citations" key="citations">
                    <small>参考资料</small>
                    {event.citations.map((c, j) => (
                      <div key={c.chunkId}>
                        ...
                        <span>{Math.round(c.score * 100)}% 匹配</span>
                      </div>
                    ))}
                  </div>
                ) : null
      ...
      <div className="composer">
        ...
        <small>
          答案由 AI 生成，请结合引用资料判断 · Enter 发送 / Shift + Enter 换行
        </small>
      </div>
    </section>
  );
...
function AgentSteps({ events }: { events: AgentEvent[] }) {
  ...
  return (
    <div className="steps multi-agent">
      <div className="graph-title">
        <span>
          <Workflow />
        </span>
        <p>
          <b>LangGraph Multi-Agent</b>
          <small>条件路由 · 有限质量回路</small>
        </p>
      </div>
      {agents.map((event, index) =>
        event.type === "agent" ? (
          <div key={`${event.role}-${index}`}>
            <span>{icons[event.role]}</span>
            <p>
              <b>{labels[event.role]}</b>
      ...
      {answer?.type === "answer" && (
        <div>
          ...
          <p>
            <b>Final Answer</b>
            <small>
              {answer.latencyMs}ms · {answer.citations.length} 条引用
            </small>
          </p>
          <Check />
        </div>
    ...
    <section className="page-content">
      ...
      <div className="stat-row">
        ...
        <Stat
          icon={<Database />}
          label="累计字符"
          value={docs.reduce((a, d) => a + d.characters, 0).toLocaleString()}
        />
        <Stat icon={<Gauge />} label="索引状态" value="Ready" />
      </div>
      <div className="doc-grid">
        {docs.map((doc) => (
          <article key={doc.id}>
            ...
            <h3>{doc.title}</h3>
            <p>{doc.characters} 字符 · 已完成向量化索引</p>
            <footer>
              <span>
                <i /> 可检索
              </span>
      ...
      {modal && (
        <div className="modal-bg">
          <div className="modal">
            ...
            <h2>添加业务知识</h2>
            <p>内容将自动切片、提取索引并进入 RAG 检索链路。</p>
            <label>
              文档标题
              <input
                value={form.title}
                onChange={(e) => setForm({ ...form, title: e.target.value })}
                placeholder="例如：售后服务规范"
              />
            </label>
            <label>
              资料来源
            ...
            <button
              ...
              onClick={save}
              disabled={
                saving || form.title.length < 2 || form.content.length < 20
              }
            >
              {saving ? <LoaderCircle /> : <FilePlus2 />}写入知识库
            </button>
          </div>
        </div>
      )}
  ...
  return (
    <section className="page-content">
      <div className="stat-row four">
        ...
        />
        <Stat icon={<Check />} label="成功率" value={`${success}%`} />
      </div>
      <div className="observability">
          ...
          {traces.length ? (
            traces.map((trace) => (
              <article className="trace" key={trace.id}>
                ...
                <div>
                  <b>{trace.query}</b>
                  <p>Planner → knowledge_search → Answer</p>
                </div>
            ...
            <div className="empty">
              <Activity />
              <p>还没有调用记录</p>
              <small>发起一次智能问答后，Trace 将显示在这里</small>
            </div>
          )}
        </div>
        <div className="quality">
          ...
          <div
            ...
            style={
              {
                "--value": `${Math.min(100, (metrics?.chunks ?? 0) * 8)}%`,
          ...
          <ul>
            <li>
              <span>知识文档</span>
              <b>{metrics?.documents ?? 0}</b>
            </li>
            <li>
              <span>缓存条目</span>
              <b>{metrics?.cacheEntries ?? 0}</b>
            </li>
            <li>
              <span>持久记忆</span>
              <b>SQLite WAL</b>
