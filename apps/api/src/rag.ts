...
export class RagService {
  ...
  search(query: string, limit = 4): Citation[] {
    ...
    return this.db.chunks.map((row) => {
      ...
      const title = this.db.documents.find((document) => document.id === row.documentId)?.title ?? "未知文档";
      return { chunkId: row.id, documentId: row.documentId, title, content: row.content, score: coverage * 0.75 + density * 0.25 };
